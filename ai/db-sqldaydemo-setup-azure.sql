...
SELECT
    ...
    CONCAT(
        s.Title, NCHAR(10),
        COALESCE(s.[Description], N''), NCHAR(10),
        N'Event: ', e.EventName, N' ', e.EditionYear, NCHAR(10),
        N'Track: ', COALESCE(t.TrackName, N''), NCHAR(10),
        N'Level: ', COALESCE(l.LevelName, N''), NCHAR(10),
        N'Language: ', COALESCE(lang.LanguageName, N''), NCHAR(10),
        N'Speakers: ', COALESCE(sp.Speakers, N''), NCHAR(10),
        N'Tags: ', COALESCE(tg.Tags, N'')
    ) AS SearchDocument
FROM dbo.SessionDetails AS s
JOIN dbo.EventEdition AS e ON e.EventEditionId = s.EventEditionId
LEFT JOIN dbo.TopicTrack AS t ON t.TopicTrackId = s.TopicTrackId
LEFT JOIN dbo.SessionLevel AS l ON l.SessionLevelId = s.SessionLevelId
LEFT JOIN dbo.Language AS lang ON lang.LanguageId = s.LanguageId
OUTER APPLY (
    SELECT STRING_AGG(CONVERT(nvarchar(max), p.DisplayName), N', ') WITHIN GROUP (ORDER BY ss.SpeakerOrder) AS Speakers
    FROM dbo.SessionSpeaker AS ss
    JOIN dbo.Speaker AS p ON p.SpeakerId = ss.SpeakerId
    WHERE ss.SessionId = s.SessionId
) AS sp
OUTER APPLY (
    SELECT STRING_AGG(CONVERT(nvarchar(max), x.TagName), N', ') WITHIN GROUP (ORDER BY x.TagName) AS Tags
    FROM dbo.SessionTag AS st
    JOIN dbo.Tag AS x ON x.TagId = st.TagId
